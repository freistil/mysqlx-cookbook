...
describe 'mysqlx::server' do
  ...
  #   node['mysqlx']['server']['packages'].each do |package_name|
  #     package(package_name).must_be_installed
  #   end
  # end

  # it 'has a config directory' do
  #   directory(node['mysqlx']['confd_dir']).must_exist.with(:owner, 'mysql').and(:group, 'mysql')
  # end

  # it 'runs as a daemon' do
  #   service(node['mysqlx']['service_name']).must_be_running
  # end

  # it 'creates a my.cnf' do
  #   file("#{node['mysqlx']['conf_dir']}/my.cnf").must_exist
  # end

  # describe 'debian' do
  #   it 'creates a config file for service control' do
  #     skip unless ['debian'].include?(node['platform_family'])
  #     file("#{node['mysqlx']['conf_dir']}/debian.cnf").must_exist
  #   end
  # end
end
