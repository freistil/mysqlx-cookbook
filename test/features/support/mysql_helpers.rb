def query(sql)
  ssh_command = "ssh " \
    ...
    "root@#{test_client_host}"
  cmd_prefix = if test_client_host
                 ssh_command
               else
                 ""
               end
  result = "echo \"#{sql}\" | " \
    "#{cmd_prefix} mysql --host=#{test_server_host} --user=#{test_user} " \
    "--password=#{test_password} --skip-column-names #{test_database}"
